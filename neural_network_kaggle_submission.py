...
def read_LSTM(reset_data, max_length):
    ...
    print(vocab_size)
    return word_dict, lemma_dict, test_word_sequences, qid, x_train, x_test, y_train, y_test, vocab_size
...
def do_LSTM(word_dict, lemma_dict, actual_test, x_train, x_test, y_train, y_test, vocab_size, max_question_length):
    ...
    model = Sequential()
    model.add(Embedding(vocab_size+1, embed_size, weights=[embedding_matrix], input_length=max_question_length, trainable=False))
    model.add(LSTM(100))
...
def main():
    ...
    reset_data = True
    if lstm:
        word_dict, lemma_dict, actual_test, qid, x_train, x_test, y_train, y_test, vocab_size = read_LSTM(reset_data, max_length)
    else:
        return
    print("starting training")
    start = time.time()
    # use actual_test instead of x_test to do real predictions
    # tes = do_LSTM(x_train, x_test, y_train, y_test, vocab_size)
    actual_pred, tes = do_LSTM(word_dict, lemma_dict, actual_test, x_train, x_test, y_train, y_test, vocab_size, max_length)
